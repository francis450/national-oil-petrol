<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-bold text-white mb-1">Customer Payments</h1>
      <p class="text-gray-400">Canonical ERPNext customer payment entries created from receivable settlement flows.</p>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,1fr)] gap-6">
      <section class="bg-gray-900 border border-gray-800 rounded-lg p-6 space-y-4">
        <div class="flex items-center justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-gray-400">Recent ERPNext customer receipts, ordered from newest to oldest.</p>
          </div>
          <button
            @click="loadPayments"
            :disabled="loading"
        ...
        <div v-if="loading" class="text-sm text-gray-400 py-8 text-center">Loading customer payments...</div>

        <div
          v-else-if="payments.length === 0"
          class="text-sm text-gray-400 py-8 text-center border border-dashed border-gray-800 rounded-lg"
        >
          No customer payment entries found yet.
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3 text-green-300 font-medium">{{ formatCurrency(payment.received_amount) }}</td>
                <td class="px-4 py-3">
      ...
      <aside class="bg-gray-900 border border-gray-800 rounded-lg p-6 space-y-4">
        <div>
          ...
          <p class="text-sm text-gray-400">Use this to confirm settlement mode, company, and draft-versus-submitted state.</p>
        </div>
          ...
          <div class="rounded-lg border border-gray-800 bg-gray-950 p-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <p class="text-green-300">{{ formatCurrency(selectedPayment.received_amount) }}</p>
              </div>
              <div>
            ...
            <div class="rounded-lg border border-gray-800 bg-black p-3 text-sm text-gray-300">
              Draft payment entries created from the new frontend should appear here immediately, then move through normal ERPNext review and submission.
            </div>
          </div>
        </template>
      </aside>
...
const loadPayments = async () => {
  ...
  try {
    payments.value = await commerceApi.listReceivablePayments()
    if (!selectedPayment.value && payments.value.length > 0) {
      selectedPayment.value = payments.value[0]
    }
  } catch (error: any) {
    pageError.value = error?.response?.data?.message || error?.message || 'Failed to load customer payments.'
  } finally {
    loading.value = false
  }
}
